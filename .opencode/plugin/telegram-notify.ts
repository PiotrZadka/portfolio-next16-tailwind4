    ...
    async event(input) {
      ...
      if (input.event.type === "message.updated") {
        ...
        const messageContent =
          (input.event as any).message?.content ||
          (input.event as any).content ||
          "";

        // Check if it's a command to send last message
        if (
          messageContent.includes("/send-last") ||
          messageContent.includes("/last")
        ) {
          if (lastMessage) {
        ...
        if (
          messageContent.includes("/send-to-phone") ||
      ...
      if (input.event.type === "message.part.updated") {
        ...
        try {
          const partContent =
            (input.event as any).part?.content ||
            (input.event as any).content ||
