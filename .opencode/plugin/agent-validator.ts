 ...
 */
export const AgentValidatorPlugin: Plugin = async ({
  client,
  project,
  directory,
}) => {
  ...
  return {
      ...
      if (executionTools.includes(tool)) {
        ...
        try {
          ...
          for (let i = 0; i < recentMessages.length - 1; i++) {
            ...
            if (
              role === "assistant" &&
              checkForApprovalLanguage(msg) &&
              nextRole === "user" &&
              checkForUserApproval(nextMsg)
            ) {
    ...
    tool: {
      ...
      validate_session: tool({
        description:
          "Validate that the current agent session is following its defined prompt rules and execution patterns. Returns a detailed validation report.",
        args: {
      ...
      check_approval_gates: tool({
        description:
          "Check if approval gates were properly enforced before execution operations (bash, write, edit, task). Returns compliance status.",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
          const tracker = toolExecutionTracker.get(sessionID);
      ...
      export_validation_report: tool({
        description:
          "Export a comprehensive validation report for the current session to a markdown file",
        args: {
          output_path: tool.schema
            .string()
            .optional()
            .describe(
              "Path to save the report (defaults to .tmp/validation-{sessionID}.md)"
            ),
        },
        async execute(args, context) {
          const { sessionID } = context;
          ...
          try {
            ...
            const outputPath =
              ...
              path.join(
                directory,
                `.tmp/validation-${sessionID.slice(0, 8)}.md`
              );
      ...
      analyze_delegation: tool({
        description:
          "Analyze whether delegation decisions followed the 4+ file rule and complexity criteria",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
      ...
      analyze_context_reads: tool({
        description:
          "Show all context files that were read during the session (e.g., .opencode/agent/openagent.md)",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
          ...
          const contextReads = behaviorLog.filter(
            (log) =>
              log.sessionID === sessionID && log.event === "context_file_read"
          );
          ...
          contextReads.forEach((log) => {
            ...
            fileReadCounts.set(
              filePath,
              (fileReadCounts.get(filePath) || 0) + 1
            );
          });

          // Sort by read count (most read first)
          const sorted = Array.from(fileReadCounts.entries()).sort(
            (a, b) => b[1] - a[1]
          );
      ...
      check_context_compliance: tool({
        description:
          "Check if required context files were read BEFORE executing tasks (e.g., read docs.md before writing documentation)",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
          ...
          const messages = messagesResponse.data || [];
          const sessionBehaviorLog = behaviorLog.filter(
            (log) => log.sessionID === sessionID
          );

          const checks = analyzeContextLoadingCompliance(
            messages,
            sessionBehaviorLog
          );
      ...
      analyze_agent_usage: tool({
        description:
          "Show which agents were active during the session and what tools they used",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
          ...
          sessionBehaviorLog.forEach((log) => {
            ...
            // Track tool usage
            if (
              log.event === "execution_tool_called" ||
          ...
          const sortedAgents = Array.from(agentStats.entries()).sort(
          ...
          sortedAgents.forEach(([agent, stats]) => {
            ...
            const durationStr =
              duration > 0 ? `${Math.round(duration / 1000)}s` : "instant";
            ...
            if (stats.toolCalls.size > 0) {
              ...
              const sortedTools = Array.from(stats.toolCalls.entries()).sort(
                (a, b) => b[1] - a[1]
              );
      ...
      debug_validator: tool({
        description:
          "Debug tool to inspect what the validator is tracking (behavior log, messages, etc.)",
        args: {},
        async execute(args, context) {
          const { sessionID } = context;
...
async function validateSessionBehavior(input: {
  ...
  const approvalChecks = analyzeApprovalGates(
    input.messages,
    input.behaviorLog
  );
  ...
  const contextComplianceChecks = analyzeContextLoadingCompliance(
    input.messages,
    input.behaviorLog
  );
...
function analyzeDelegation(messages: any[]): ValidationCheck[] {
  ...
  for (const msg of messages) {
    ...
    const writeEditCount = tools.filter(
      (t) => t === "write" || t === "edit"
    ).length;
...
function analyzeCriticalRules(messages: any[]): ValidationCheck[] {
  ...
  for (let i = 0; i < messages.length - 1; i++) {
    ...
    if (role === "assistant" && metadata?.error) {
      ...
      const hasAutoFix = nextTools.some((t) =>
      ...
      if (hasAutoFix) {
        checks.push({
          ...
          severity: "error",
          details:
            "⛔ Auto-fix attempted after error - violates stop_on_failure rule",
...
function analyzeContextLoadingCompliance(
  messages: any[],
  behaviorLog: any[]
): ValidationCheck[] {
  const checks: ValidationCheck[] = [];
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const tools = extractToolsFromMessage(msg);
    const executionTools = tools.filter((t) =>
      ["write", "edit", "bash", "task"].includes(t)
    );
    ...
    for (const rule of contextRules) {
      const matchesTask = rule.taskKeywords.some((keyword) =>
      ...
      if (matchesTask) {
        ...
        const contextReadBefore = contextReads.some(
          (read) =>
            read.filePath.includes(rule.requiredFile) &&
            read.timestamp < msgTimestamp
        );
...
function formatDelegationAnalysis(analysis: any): string {
  ...
  if (analysis.fileCountStats.length > 0) {
    const avg =
      analysis.fileCountStats.reduce((a: number, b: number) => a + b, 0) /
      analysis.fileCountStats.length;
    lines.push(`**File count per delegation:**`);
    lines.push(`- Average: ${avg.toFixed(1)} files`);
    lines.push(
      `- Range: ${Math.min(...analysis.fileCountStats)} - ${Math.max(...analysis.fileCountStats)} files`
    );
